...
; 1: click brightness NLP
; 2: goto hue
; 3: apply settings NLP
; 4: sync photos
...
coords[3] := []         ; Initialize the sub-array for PC vertical mode
...
; Define PC horizontal coordinates
coords[2].Push({"x": 427, "y": 222}) ; open NLP: click on brightness 
coords[2].Push({"x": 419, "y": 688}) ; click on hue
coords[2].Push({"x": 302, "y": 958}) ; click on apply
coords[2].Push({"x": 144, "y": 832}) ; click on sync settings
...
; example use:
; move mouse to position 2 given mode == 2
; mode := 2
; moveMouseTo(2)
; should run MouseMove, 344, 814

F8::
{
ToolTip, % coordinates_PC[1][1]
Return
}
...
^/::
{
    ...
    Sleep 300
    moveMouseTo(1)          ; Clicks on brightness value so that pressing tab cycles through menu
    Sleep 100
    Click
    Sleep 100
